...
const Posts = () => {
  ...
  return <NavbarLayout>
      <H3 color="#2C2C2C" mb={2}>See Recent Posts</H3>
      ...
      {post.map((item, ind) => <Post post={item} key={ind} />)}
     
      <FlexBox flexWrap="wrap" justifyContent="space-between" alignItems="center" mt={4}>
        <Span color="grey.600">Showing 1-6 of {post.length} Posts</Span>
        <Pagination count={Math.ceil(post.length / 6)} variant="outlined" color="primary" />
      </FlexBox>
    </NavbarLayout>;
};
